---
title: "RNA-seq sputum sample with PCs and clusters"
...
load("/Users/carlacasanovasuarez/Documents/Master Bioinformatics UAB/Prácticas Radiomics/Radiomic features/Results_rfeatures/radar_L1_Norm_scaled_adjusted_combat_final.rda")
...
pheno_sputum <- phenoData(sputum_eset_filt)
...
annotation.sputum.ok <- as.data.frame(annotation.sputum[filter_rows,])
...
save(rdr_assay, file = "/Users/carlacasanovasuarez/Documents/Master Bioinformatics UAB/Prácticas Radiomics/Radiomic features/Results_rfeatures/R objects/rdr_assay_scaled_combat.rda")
```

```{r patients_sputum_eset, eval=FALSE}
save(sputum_eset_filt, file = "/Users/carlacasanovasuarez/Documents/Master Bioinformatics UAB/Prácticas Radiomics/Radiomic features/Results_rfeatures/R objects/sputum_eset_filtered.rda")

save(counts.ok, file = "/Users/carlacasanovasuarez/Documents/Master Bioinformatics UAB/Prácticas Radiomics/Radiomic features/Results_rfeatures/R objects/sputum_eset_countsOK_Combat.rda")
...
pcaResults2 <- PCA(t(counts.ok[selectedGenes,]),graph = FALSE)

#load("pca genes sputum.rda")
```
...
#pdf("PCA gene counts sputum by stage.pdf")
#phenoDataFrame$SEX

qplot(pcaResults2$ind$coord[,1], pcaResults2$ind$coord[,2], xlab = "PC1", ylab = "PC2", main = "Scaled PCA of patients with sputum samples", colour = phenoDataFrame$AGE) + 
  labs(colour = "Age")
...
```{r ellipses_gene_PCA}
pPCA1 <- ggpar(fviz_pca_ind(pcaResults2,
label = "none", # hide individual labels
habillage = phenoDataFrame$GROUP_bin, # color by groups palette = c("#00AFBB", "#E7B800"),
addEllipses = TRUE # Concentration ellipses
), title = "Individuals with sputum samples colored by COPD stage", font.main = c(8, "plain", "black"), legend.title = "COPD stage", caption = "A", font.caption = c(12, "bold", "black"))

pPCA2 <- ggpar(fviz_pca_ind(pcaResults2,
label = "none", # hide individual labels
habillage = as.factor(phenoDataFrame$SEX), # color by groups palette = c("#00AFBB", "#E7B800"),
addEllipses = TRUE # Concentration ellipses
), title = "Individuals with sputum samples colored by sex", font.main = c(8, "plain", "black"), legend.title = "Sex", caption = "B", font.caption = c(12, "bold", "black"))
```

Individuals contribution:  

```{r ind_cont_gene_PCA}
pPCA3 <- ggpar(fviz_pca_ind(pcaResults2, col.ind = "contrib",
             gradient.cols = c("#00AFBB", "#E7B800", "#FC4E07"),
             repel = TRUE # Avoid text overlapping (slow if many points)
             ), title = "Individuals with sputum samples colored by contribution (%)", font.main = c(7, "plain", "black"), caption = "C", font.caption = c(12, "bold", "black"))
```
...
ggsave("PCA sputum individuals.pdf",pAll, width=6, height=6, units="cm", scale=3.5)
```

PCA for genes:  

```{r PCA_genes_plot}
qplot(pcaResults2$var$coord[,1], pcaResults2$var$cor[,2], xlab = "PC1", ylab = "PC2", main = "Scaled PCA of genes")
```

MDS plots:  

```{r plot_MDS}
color.gr <- ifelse(phenoDataFrame$GROUP_bin=="Moderate", "orange", "purple")
color.cough <- ifelse(phenoDataFrame$COUGH=="Chronic cough", "red", "green")
color.sex <- ifelse(phenoDataFrame$SEX=="F", "orange", "purple")
#color.sex <- ifelse(phenoDataFrame$SEX=="F","blue", "coral")

pdf("MDS plots sputum.pdf")
par(mfrow=c(2,1))

plotMDS(y, col= color.gr)
legend("bottomright",fill=c("orange", "purple"),legend=levels(phenoDataFrame$GROUP_bin))
title("MDS plot of individuals with sputum samples by COPD stage")
...
legend("bottomright",fill=c("orange","purple"),legend=levels(as.factor(phenoDataFrame$SEX)))
title("MDS plot of individuals with sputum samples by sex")

dev.off()
```

As can be seen in the previous analysis, in this case patients are slightly stratified by COPD stage. Nevertheless, `Moderate` and `Severe` seem to be close stages regarding gene expression since patients are almost mixed. 
...
highly_variable_lcpm <- counts.ok[selectedGenes,]
```
...
# Plot heatmap
aheatmap(highly_variable_lcpm,col=rev(morecols(50)),main="Top 500 most variable genes across sputum samples",annCol=phenoDataFrame[,c("GROUP_bin", "SEX")],labCol=phenoDataFrame[,"GROUP_bin"], scale="row")
...
```{r contrib_var}
#pdf("Contribution variables in PCs with DE genes (ComBat).pdf")

# Contributions of variables to PC1
ggpar(fviz_contrib(res.pca, choice = "var", axes = 1, top = 10, ggtheme = theme_classic()), x.text.angle=90)
# Contributions of variables to PC2
ggpar(fviz_contrib(res.pca, choice = "var", axes = 2, top = 10, ggtheme = theme_classic()), x.text.angle=90)
# Contributions of variables to PC3
ggpar(fviz_contrib(res.pca, choice = "var", axes = 3, top = 10, ggtheme = theme_classic()),x.text.angle=90)
...
```{r predict_prob, eval=FALSE}
set.seed(1234)

rdr_assay.T <- as.data.frame(t(rdr_assay))
identical(rownames(phenoDataFrame), rownames(rdr_assay.T))

rdr_assay.T$GROUP <- phenoDataFrame$GROUP_bin
#names(rdr_assay.T)[names(rdr_assay.T) == '10Percentile.original'] <- 'tenPercentile.original'
#names(rdr_assay.T)[names(rdr_assay.T) == '90Percentile.original'] <- 'ninetyPercentile.original'

#confusionMatrix(predict(m3, rdr_assay.T), rdr_assay.T$GROUP)
#Probabilities
#pred <- predict(m3, rdr_assay.T, type = "prob")

 pred_real <- predict.gbm(object = model_gbm,
                   newdata = rdr_assay.T,
                   n.trees = 500,           # 500 tress to be built
                   type = "response")
  
  #### Create a confusion matrix:
  class_names <- colnames(pred_real)[apply(pred_real, 1, which.max)]
  result <- data.frame(rdr_assay.T$GROUP, class_names)
  conf_mat <- confusionMatrix(as.factor(rdr_assay.T$GROUP), as.factor(class_names))
```
...
variables_interest <- data.frame(Sex = as.factor(phenoDataFrame$SEX), 
                                 ...
                                 pack_years_smoke = phenoDataFrame$SUPKYR,
...
# Do not consider PCs 
mod.fow <- stats::step(lm(t(countData) ~ ., data = variables_interest[,1:(ncol(variables_interest)-22)]), trace = FALSE, direction = "forward")
...
not.sig.variables <- which(variables.sig == FALSE)
not.sig.variables
```
...
# Backward Dwalk and FEV1PSPC but + Cough
mod.sagfdac <- lm(t(countData) ~ Group + Sex + Age + fume_expose + dusty_expose + history_asthma + Cough, data = variables_interest)

# ALL + Dwalk and FEV1PSPC
mod.dsagffdac <- lm(t(countData) ~ Group + Sex + Age + Dwalk + FEV1PSPC + fume_expose + dusty_expose + history_asthma + Cough, data = variables_interest)
...
# All + BMI +  Cr_wheezeng 
mod.dsagffdacBW <- lm(t(countData) ~ Group + Sex + Age + Dwalk + FEV1PSPC + fume_expose + dusty_expose + history_asthma + Cough + BMI + Cr_wheezeng, data = variables_interest)

# All + pack years + cardiac history
mod.dsagffdacBWPKh <- lm(t(countData) ~ Group + Sex + Age + Dwalk + FEV1PSPC + fume_expose + dusty_expose + history_asthma + Cough + BMI + Cr_wheezeng + pack_years_smoke + heart_failure, data = variables_interest)

# All + backward dusty
mod.dsagffacBWPKh <- lm(t(countData) ~ Group + Sex + Age + Dwalk + FEV1PSPC + fume_expose + history_asthma + Cough + BMI + Cr_wheezeng + pack_years_smoke + heart_failure, data = variables_interest)

# Choice model 
mod.final <- lm(t(countData) ~ Sex + Age + BMI + pack_years_smoke, data = variables_interest)
...
rmse(fitted(mod.sagfdac), t(countData))
rmse(fitted(mod.dsagffdac), t(countData))

# Best model
rmse(fitted(mod.dsagffdacBW), t(countData))
rmse(fitted(mod.dsagffdacBWPKh), t(countData))
rmse(fitted(mod.dsagffacBWPKh), t(countData))
rmse(fitted(mod.final), t(countData))
```
...
```{r sva_avalysis}
# PC1_rdr + PC2_rdr + PC3_rdr + PC4_rdr + PC5_rdr + PC6_rdr + PC7_rdr + PC8_rdr + PC9_rdr + PC10_rdr + PC11_rdr + PC12_rdr + PC13_rdr + PC14_rdr + PC15_rdr

#  + PC16_rdr + PC17_rdr + PC18_rdr + PC19_rdr + PC20_rdr + PC21_rdr + PC22_rdr

# Group + Sex + Age + Dwalk + FEV1PSPC + fume_expose + dusty_expose + history_asthma + Cough + BMI + Cr_wheezeng +
# Group + Sex + Age + Dwalk + BMI + pack_years_smoke
...
design <- model.matrix( ~ Sex + Age + BMI + Years_smoke + PC1_rdr + PC2_rdr + PC3_rdr + PC4_rdr + PC5_rdr + PC6_rdr + PC7_rdr + PC8_rdr + PC9_rdr + PC10_rdr + PC11_rdr + PC12_rdr + PC13_rdr + PC14_rdr + PC15_rdr + PC16_rdr + PC17_rdr + V1 + V2 + V3 + V4 + V5 + V6 + V7 + V8 + V9 + V10 + V11 + V12 + V13 + V14 + V15 + V16 + V17, data=variables_interest_sva)
...
# P-value distribution of results computed by limma
ggplot(data = p.val.voom, aes(x = PC11_rdr)) +
geom_histogram(bins = 100)

# P-value distribution of results computed by limma
ggplot(data = p.val.voom, aes(x = PC15_rdr)) +
geom_histogram(bins = 100)
```
...
# QQplot plot for p-values computed by limma
GWASTools::qqPlot(p.val.voom$PC11_rdr)

# QQplot plot for p-values computed by limma
GWASTools::qqPlot(p.val.voom$PC15_rdr)
```
...
labels.PC2 <- ifelse(annotation.sputum.ok[rownames(groupPC2),"SYMBOL"] != "", annotation.sputum.ok[rownames(groupPC2),"SYMBOL"], rownames(groupPC2))

volcano2 <- EnhancedVolcano(groupPC2, lab = labels.PC2, 
                x = 'logFC',
                ...
                pointSize = 3.0,
                labSize = 4.0,
                title = "Genes from sputum DE in PC2 with relaxed p-value (10e-5)",
                titleLabSize = 20)
```
...
labels.PC4 <- ifelse(annotation.sputum.ok[rownames(groupPC4),"SYMBOL"] != "", annotation.sputum.ok[rownames(groupPC4),"SYMBOL"], rownames(groupPC4))

volcano4 <- EnhancedVolcano(groupPC4, lab = labels.PC4, 
                x = 'logFC',
                ...
                pointSize = 3.0,
                labSize = 4.0,
                title = "Genes from sputum DE in PC4 with relaxed p-value (10e-5)",
                titleLabSize = 20)
```
...
```{r contrast_rf_pc9}
groupPC9 <- topTable(fit, number = dim(counts.ok)[1], coef="PC9_rdr")
groupPC9 <- groupPC9[order(groupPC9$P.Value),]
head(groupPC9)
```

```{r volcano_rf_pc9, fig.align='center', fig.height=8}
labels.PC9 <- ifelse(annotation.sputum.ok[rownames(groupPC9),"SYMBOL"] != "", annotation.sputum.ok[rownames(groupPC9),"SYMBOL"], rownames(groupPC9))

volcano9 <- EnhancedVolcano(groupPC9, lab = labels.PC9, 
                x = 'logFC',
                y = 'P.Value', ylim=c(0,20),
                pCutoff = 10e-5,
                FCcutoff = 0.05,
                pointSize = 3.0,
                labSize = 4.0,
                title = "Genes from sputum DE in PC9 with relaxed p-value (10e-5)",
                titleLabSize = 20)
```
...
```{r contrast_rf_pc11}
groupPC11 <- topTable(fit, number = dim(counts.ok)[1], coef="PC11_rdr")
groupPC11 <- groupPC11[order(groupPC11$P.Value),]
head(groupPC11)
```

```{r volcano_rf_pc11, fig.align='center', fig.height=8}
labels.PC11 <- ifelse(annotation.sputum.ok[rownames(groupPC11),"SYMBOL"] != "", annotation.sputum.ok[rownames(groupPC11),"SYMBOL"], rownames(groupPC11))

volcano11 <- EnhancedVolcano(groupPC11, lab = labels.PC11, 
                x = 'logFC',
                y = 'P.Value', ylim=c(0,20),
                pCutoff = 10e-5,
                FCcutoff = 0.05,
                pointSize = 3.0,
                labSize = 4.0,
                title = "Genes from sputum DE in PC11 with relaxed p-value (10e-5)",
                titleLabSize = 20)
```
...
```{r contrast_rf_pc14}
groupPC14 <- topTable(fit, number = dim(counts.ok)[1], coef="PC14_rdr")
groupPC14 <- groupPC14[order(groupPC14$P.Value),]
head(groupPC14)
```

```{r volcano_rf_pc14, fig.align='center', fig.height=8}
labels.PC14 <- ifelse(annotation.sputum.ok[rownames(groupPC14),"SYMBOL"] != "", annotation.sputum.ok[rownames(groupPC14),"SYMBOL"], rownames(groupPC14))

volcano14 <- EnhancedVolcano(groupPC14, lab = labels.PC14, 
                x = 'logFC',
                y = 'P.Value', ylim=c(0,20),
                pCutoff = 10e-5,
                FCcutoff = 0.05,
                pointSize = 3.0,
                labSize = 4.0,
                title = "Genes from sputum DE in PC14 with relaxed p-value (10e-5)",
                titleLabSize = 20)
```
...
```{r contrast_rf_pc15}
groupPC15 <- topTable(fit, number = dim(counts.ok)[1], coef="PC15_rdr")
groupPC15 <- groupPC15[order(groupPC15$P.Value),]
head(groupPC15)
```

```{r volcano_rf_pc15, fig.align='center', fig.height=8}
labels.PC15 <- ifelse(annotation.sputum.ok[rownames(groupPC15),"SYMBOL"] != "", annotation.sputum.ok[rownames(groupPC15),"SYMBOL"], rownames(groupPC15))

volcano15 <- EnhancedVolcano(groupPC15, lab = labels.PC15, 
                x = 'logFC',
                y = 'P.Value', ylim=c(0,20),
                pCutoff = 10e-5,
                FCcutoff = 0.05,
                pointSize = 3.0,
                labSize = 4.0,
                title = "Genes from sputum DE in PC15 with relaxed p-value (10e-5)",
                titleLabSize = 20)
```
...
```{r all_volcano}
volcanoAll_ann <- plot_grid(volcano2, volcano11, volcano14, volcano15, labels=c("A", "B", "C", "D"), ncol = 2, label_size = 20)
volcano_rest <- plot_grid(volcano4, volcano9, labels=c("E", "F"), ncol = 2, label_size = 20)

ggsave("Volcano plots sputum annotated genes.pdf", volcanoAll_ann, width=10, height=9, units="cm", scale=6)
ggsave("Volcano plots sputum rest.pdf", volcano_rest, width=8, height=5, units="cm", scale=6)
```
...
```{r differentially_expressed_pc11}
# Store genes with p.adjusted lesser than 0.01 (highly significant)
mask <- groupPC11$adj.P.Val < 0.05 & !is.na(groupPC11$adj.P.Val)
deGenes <- rownames(groupPC11[mask, ]) 
...
```{r differentially_expressed_pc15}
# Store genes with p.adjusted lesser than 0.01 (highly significant)
mask <- groupPC15$adj.P.Val < 0.05 & !is.na(groupPC15$adj.P.Val)
deGenes <- rownames(groupPC15[mask, ]) 
...
```{r differentially_expressed_pc15}
# Store genes with p.adjusted lesser than 0.01 (highly significant)
mask <- groupPC8$adj.P.Val < 0.05 & !is.na(groupPC8$adj.P.Val)
deGenes <- rownames(groupPC8[mask, ]) 
...
```{r differentially_expressed_pc15}
# Store genes with p.adjusted lesser than 0.01 (highly significant)
mask <- groupPC14$adj.P.Val < 0.05 & !is.na(groupPC14$adj.P.Val)
deGenes <- rownames(groupPC14[mask, ]) 
...
```{r gene_universe}
geneUniverse <- rownames(groupPC11[!is.na(groupPC11$P.Value), ]) 
length(geneUniverse)
```
...
ans.go <- enrichGO(gene = deGenes.entrez, ont = "BP",
                   universe = geneUniverse.entrez,
                   OrgDb ="org.Hs.eg.db", 
                   readable=TRUE, pvalueCutoff = 0.05)

# See results
tab.go <- as.data.frame(ans.go@result) 
...
FOX# See results
tab.kegg <- as.data.frame(ans.kegg@result)
...
Start enrichment checking if genes related with respiratory diseases are present in the data set:  
```{r enrichment_gene_universe}
# Diseases enrichment analysis using gene universe
res_enrich.universe <-disease_enrichment(entities = geneUniverse.entrez, vocabulary = "ENTREZ", database = "CURATED")

table_gene_universe <- res_enrich.universe@qresult

# How many genes match respiratory diseases from our sample in curated data bases?
length(grep("Respiratory", table_gene_universe$disease_class_name))
```
...
Plot significant enrichments in PC15:  

```{r PC_enrichment_plots, eval=FALSE}
library(enrichplot)

# GO
dotplot(ans.go, showCategory=20) + ggtitle("GO PC2") 
...
#plot(res_enrich, class = "Enrichment",  cutoff= 0.05) 
```


## DE using cluster  

Prepare clusters with more than 2 radiomic features and compute the mean as cluster value for each patient:  
...
Compute selection of variables:  
...
Perform DE analysis with voom using clusters:  
